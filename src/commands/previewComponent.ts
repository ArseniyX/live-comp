...
export async function previewComponent(
	...
	if (!['.tsx', '.jsx', '.ts', '.js'].includes(ext)) {
		vscode.window.showErrorMessage(
			'Please select a React component file (.tsx, .jsx, .ts, or .js)'
		);
		return;
	}
	...
	if (!workspaceFolder) {
		vscode.window.showErrorMessage(
			'Could not determine workspace folder. Please open a workspace.'
		);
		return;
	}
...
export function registerPreviewCommand(context: vscode.ExtensionContext): vscode.Disposable {
	return vscode.commands.registerCommand('live-comp.previewComponent', (uri?: vscode.Uri) =>
		previewComponent(context, uri)
	);
}
